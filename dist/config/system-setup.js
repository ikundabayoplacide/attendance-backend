...
const setupSystem = async () => {
    try {
        ...
        try {
            ...
            await database_1.default.getQueryInterface().dropAllTables();
        ...
        try {
            ...
            await database_1.default.getQueryInterface().dropTable('user_roles');
        ...
        try {
            await database_1.default.getQueryInterface().dropTable('role_permissions');
        ...
        try {
            await database_1.default.getQueryInterface().dropTable('users');
        ...
        try {
            await database_1.default.getQueryInterface().dropTable('roles');
        ...
        try {
            await database_1.default.getQueryInterface().dropTable('permissions');
        ...
        for (const permission of permissions) {
            try {
                ...
                const newPermission = await models_1.default.Permission.create({
        ...
        for (const roleData of roles) {
            try {
                ...
                const newRole = await models_1.default.Role.create({
                ...
                const rolePermissions = await models_1.default.Permission.findAll({
                    where: { name: roleData.permissions }
                });
                if (rolePermissions.length > 0) {
                    //We use setPermissions to create associations in the join table at once
                    await newRole.setPermissions(rolePermissions);
        ...
        const ownerUser = await models_1.default.User.create({
            fullName: 'System Owner',
            email: ownerEmail,
            phoneNumber: phoneNumber,
            password: await (0, bcrypt_1.hash)(ownerPassword, 10),
            status: 'active'
        });
        console.log(`✅ Created default system owner user with email: ${ownerEmail}`);
        // Assign owner role to the user
        const ownerRole = await models_1.default.Role.findOne({ where: { name: 'system_owner' } });
        if (ownerRole) {
            await ownerUser.setRoles([ownerRole]);
